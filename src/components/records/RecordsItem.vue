...
const checkValues = handleSubmit((values) => {
  if (!errors.value.badges && !errors.value.type && !errors.value.login && !errors.value.password) {
    ...
    recordsStore.setRecord(record.id, { ...values, badges, password });
  }
});
</script>

<template>
  <form @submit.prevent class="grid grid-cols-[repeat(4,_minmax(100px,_1fr))_36px] gap-2">
    <div>
      <UIInput
        v-model="badges"
    ...
    <RecordsRemoveButton class="h-9" @click="recordsStore.removeRecord(record.id)" />
  </form>
</template>
